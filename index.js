...
const MenchiesOrder = require("./MechiesOrder1");
...
app.get("/payment/:phone", (req, res) => {
  ...
  if (!oOrders.hasOwnProperty(sFrom)) {
    ...
    const items = oOrders[sFrom].totalItems.join(", ")
    
    const price = oOrders[sFrom].totalPrice
    res.end(oOrders[sFrom].renderForm(items,price));
  }
});
...
app.post("/payment", (req, res) => {
  ...
  oOrders[sFrom] = new MenchiesOrder(sFrom);
...
app.post("/sms", (req, res) => {
  ...
  if (!oOrders.hasOwnProperty(sFrom)) {
    oOrders[sFrom] = new MenchiesOrder(sFrom, sUrl);
  }
  if (oOrders[sFrom].isDone()) {
    delete oOrders[sFrom];
  }
